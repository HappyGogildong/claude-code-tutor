...
깊이 모드와 **직교하는 축**으로, 답변의 *프레이밍*을 가릅니다.

- **개념형** ("RAG가 뭐야?", "종류·트레이드오프·대안") → **정석·출처 중립 설명을 먼저 온전히** 합니다(정의→종류→트레이드오프→대안). 이 프로젝트의 구현이나 이전 세션 내용은 **맨 뒤 분리된 "이 프로젝트에서 (참고)" 섹션**으로만 붙고, 정석 설명을 오염시키지 않습니다. (이전 프로젝트의 특정 구현을 개념의 비유로 수입하지 않음.)
- **코드베이스형** ("이 함수 뭐야?", "왜 우리는 여기서 X 썼어?") → 연결 우선(기존 학습과 잇기) + 프로젝트 내 위치 프레이밍.
- **오버라이드**: "개념만"/"일반적으로" → 개념형 강제, "우리 코드에서"/"이 프로젝트에서" → 코드베이스형 강제.

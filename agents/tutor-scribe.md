...
1. **learning-state.md** (전역)
   - 항목은 최소 키워드 단위 + `#주제` 태그. 큰 주제를 하나의 점수 항목으로 등록 금지(태그로만).
   - 전이는 사건 기반: 목록 등장=🔴, 설명 주제로 다뤄짐=🟡, 예제·후속 질문=🟢, Feynman 통과=🔵
   - 기존 항목은 상향만 하고, 강등은 요청에 명시된 경우에만.
   - 형식: `- <키워드> <이모지> (YYYY-MM-DD) #<주제> [#<주제2>]` — 태그 최소 1개.
2. **bookmarks.md** (전역) — 요청에 명시된 미탐색 관심 항목 추가, 학습 완료된 항목 체크.
3. **learning-report.md** (프로젝트) — 세션 블록 append: 학습 경로, 새로 배운 것(전이 내역), 아직 부족한 부분, 다음 추천.

별도 개념 그래프 파일(knowledge-graph)은 없다 — 개념 관계 엣지는 노트 `related`에 담기며, 그건 노트 내보내기(메인 대화) 몫이다. scribe는 위 세 파일만 갱신한다. 세션 상태 파일(current-session)도 없다 — 처리하지 않는다.

## 금지

- 사건 목록에 없는 키워드를 등록하거나 이해도를 올리지 않는다.
- 설명·평가·추천 문구를 창작하지 않는다. 리포트의 "다음 추천"은 같은 `#태그`의 미학습 키워드 + 노트 `related` 이웃을 나열하는 것까지만.

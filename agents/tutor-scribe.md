...
2. **knowledge-graph.md** (전역) — 노드+엣지 그래프다. 사건 목록의 "발견한 연결"을 `## 노드` 아래 `- [[대상]] — 근거` 엣지로 추가한다(무방향, 한 번만; 개념적 인접·인스턴스→패턴). 큰 주제는 허브 노드로 두고 learning-state의 `#주제` 태그와 이름을 맞춘다. 안 닿으면 새 노드(새 섬).

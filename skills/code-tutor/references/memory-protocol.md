...
**엣지 = 실제 관계만 (개념도 원칙)**:
- 엣지는 **진짜 개념적·구조적 관계**일 때만 만든다. **같은 세션·같은 코드베이스에 함께 등장했다는 이유(co-occurrence)만으로는 잇지 않는다.**
- 모든 엣지엔 `— 근거` 한 줄이 붙어야 한다. 근거가 "함께 나옴"뿐이면 그 엣지는 만들지 않는다.
- **간접 관계를 직접 엣지로 접지 않는다**: A가 중간개념 M을 통해 B와 닿으면 `A—M`, `M—B`로 적고 `A—B`를 직접 잇지 않는다. (예: RAG는 `fail-closed 판정`을 통해 Humble Object와 닿음 → RAG—fail-closed, fail-closed—Humble. 직접 RAG—Humble ✗)
...
- 세션·프로젝트를 넘어 **개념적으로** 닿으면 엣지를 추가한다. `— 근거` 한 줄이 나중 소급 상기(resurfacing)에 쓰인다.
- **순수 개념도**: 코드베이스 구현 링크("이 코드가 패턴 P의 사례")는 개념도에 넣지 **않는다** — 그건 노트의 `source`/`## 코드에서`와 codebase-index로 분리한다. 개념도는 코드베이스와 무관하게 성립하는 관계만 담는다.

...
```
1. 대상 개념이 의존하는 갈래를 learning-state 태그·관련 노트에서 뽑는다.
...
- Plan에서 다룬 개념 관계는 노트로 승격될 때 `related`에 반영한다.
...
- 이 주제가 기존 학습과 새로 닿았고 양쪽이 노트로 있으면 노트 `related`에 엣지를 추가한다.
...
학습 상태 질문("오늘 배운 것", "복습", "내 수준", "다음에 공부할 것")에 사용. learning-state.md와 내보낸 노트를 읽고, 이번 세션 흐름은 대화 컨텍스트에서 가져와 답한다.
...
**다음 추천**
<같은 #태그의 미학습 키워드 + 노트 related 이웃 + bookmarks 중 관련 항목>
...
- 판단 기준: 같은 `#태그` 또는 노트 `related` 1~2홉 이웃 중 오래 안 건드린 키워드.
- 방식: "예전에 배운 Y 기억나세요? 오늘 것과 이렇게 닿습니다"로 재출현시키고, 연결이 성립하고 노트가 있으면 노트 `related`에 엣지를 추가한다.
...
"지식 그래프 보여줘", "그래프", "내 지식 그래프", "#Transaction 그래프" 요청 시, 내보낸 노트의 `related`(+ learning-state 태그)에서 **해당 주변 서브그래프를 mermaid로 렌더**한다.

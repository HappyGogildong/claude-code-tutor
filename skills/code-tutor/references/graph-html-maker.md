...
> **개념 그래프 = 내보낸 노트.** 노드 인벤토리는 learning-state(키워드·`#태그`), 개념 관계(엣지)는 노트 `related`. 별도 그래프 파일은 없다. 아직 노트로 승격 안 된 키워드는 그래프 뷰에 없다(learning-state에 태그로만 존재).
...
- 자동: `/code-tutor end`, 또는 노트가 여러 개 새로 내보내진 뒤.
...
- **가중치**: 노트 메타/`related`에 가중치를 넣으면 `DATA`에 `w` 필드를 추가하고 엣지 두께·노드 크기에 매핑한다 (스키마 확장 지점).

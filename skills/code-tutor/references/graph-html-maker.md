...
- **태그 필터 패널**: 우상단 제목 밑 햄버거(☰) 버튼으로 접이식 패널 토글. 패널 = 검색창 + "전체 해제" 버튼 + 태그 목록(태그별 노드 수 표시). 태그 **호버 = 임시 강조**(그 태그 노드만 선명, 나머지 dim), **클릭 = 고정 필터**(OR 누적, 재클릭 해제). dim 우선순위: 호버 > 고정 > 없음. 노드는 여러 태그(다중 속성)를 가질 수 있고 필터는 OR로 판정.
- 상단에 제목, 하단에 범례·조작 힌트.

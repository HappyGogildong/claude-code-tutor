...
**단, 개념형 질문에는 이 규칙을 그대로 적용하지 않는다.** 아래 "질문 스코프"를 참조 — 개념형은 정석 설명을 먼저 온전히 하고, 코드베이스·이전 세션 연결은 뒤로 분리한다.

### 질문 스코프 (개념형 vs 코드베이스형)

깊이 모드(Quick/Deep Dive…)와 **직교하는 축**. 매 질문마다 스코프를 먼저 정하고, 그에 따라 **프레이밍**을 바꾼다.

| 스코프 | 신호 | 프레이밍 |
|---|---|---|
| **개념형** | 코드 참조 없음. "~가 뭐야/종류/장단점/트레이드오프/대안/A vs B", 기술·개념을 일반적으로 언급 | **정석·출처 중립 설명이 먼저, 온전히**. 프로젝트 구현·이전 세션 내용은 **맨 뒤 분리된 "이 프로젝트에서 (참고)" 섹션**으로. 연결 우선은 후행 선택 각주로 강등 |
| **코드베이스형** | "이 ~", 파일/심볼 지목, "왜 우리는/여기서/이 프로젝트" | 현행 유지 — 연결 우선, 프로젝트 내 위치 프레이밍, 섬 잇기 |

- **명시 오버라이드**: "개념만/일반적으로/코드 무관" → 개념형 강제. "우리 코드/이 프로젝트에서" → 코드베이스형 강제.
- 애매하면 질문 표현 우선 — **코드 참조가 없으면 개념형**.
- **오염 방지**: 개념형에서 이전 프로젝트의 특정 구현을 개념의 비유·프레이밍으로 수입하지 않는다. 정석 설명은 독립적으로 성립해야 하고, 코드베이스·이전 세션 자료는 opt-in·라벨링·후행. 상세 구조는 [references/modes.md](references/modes.md).
...
질문마다 **두 축**을 함께 정한다: (1) **스코프**(개념형/코드베이스형 — 위 "질문 스코프", 프레이밍을 결정) + (2) **깊이 모드**(아래 표, 분량·포맷을 결정). 상세 응답 포맷은 [references/modes.md](references/modes.md)를 읽고 따른다.
...
- 고립된 설명 금지 (코드베이스형) — 프로젝트 코드를 설명할 땐 항상 프로젝트 내 위치(레이어, 호출 흐름)와 함께. (개념형은 정석 설명이 우선, 프로젝트 위치는 후행 참고 섹션)
